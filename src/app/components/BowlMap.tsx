...
import { Lock, LogOut, Plus, X, MapPin } from "lucide-react";

// Leaflet is loaded via CDN in index.html - we use window.L
declare global {
  interface Window {
    L: any;
  }
...
// We use a Google Charts QR API URL (no key required, free)
function QRCodeImage({ url, size = 160 }: { url: string; size?: number }) {
  ...
  return (
    <img
      ...
      onError={(e) => {
        // Fallback: show a placeholder if QR service is unavailable
...
export function BowlMap() {
  ...
  const [bowlLocations, setBowlLocations] = useState<BowlLocation[]>(DEFAULT_BOWLS);
  ...
  useEffect(() => {
    ...
    const map = L.map(mapDivRef.current, {
      center: [FAISAL_TOWN_CENTER.lat, FAISAL_TOWN_CENTER.lng],
      zoom: 15,
      zoomControl: true,
    });
    ...
    // Click on map to pick location when admin is adding a bowl
    map.on("click", (e: any) => {
  ...
  // Sync markers whenever bowlLocations change
  useEffect(() => {
      ...
      const marker = L.marker([bowl.lat, bowl.lng], { icon: bowlIcon })
        ...
        .bindPopup(`
          <div style="font-family: Nunito, sans-serif; min-width: 160px;">
            <div style="font-weight: 700; color: #F97316; font-size: 14px; margin-bottom: 4px;">🥣 ${bowl.name}</div>
            <div style="color: #555; font-size: 12px;">${bowl.note}</div>
          </div>
        `);
  ...
  const handleAddBowl = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    const newBowl: BowlLocation = {
      id: Math.max(...bowlLocations.map((b) => b.id)) + 1,
      name: newBowlName,
      note: newBowlNote,
      lat,
      lng,
    };
    setBowlLocations([...bowlLocations, newBowl]);
    setNewBowlName("");
    setNewBowlNote("");
  ...
  // Page URL for QR code (bowl map page)
  const mapPageUrl = typeof window !== "undefined"
    ? window.location.origin + "/"
    : "https://pawpoint-faisaltown.vercel.app/";
      ...
      <div className="relative" style={{ height: "560px" }}>
        ...
        {!mapReady && (
          <div className="absolute inset-0 flex items-center justify-center bg-green-50 z-10">
            <div className="text-center text-gray-600">
              ...
              <p className="font-bold">Loading Faisal Town Map…</p>
            </div>
          </div>
        )}
        ...
        {isAuthenticated && !showAddModal && (
          <div className="absolute top-4 left-4 z-10 bg-white/90 rounded-xl px-4 py-2 text-sm text-gray-600 shadow">
            💡 Click on the map to pick a bowl location, then press +
          </div>
        )}

        {/* Active Feeding Bowls card – bottom */}
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            {isAuthenticated && (
              <button
                ...
                className="bg-[#F97316] text-white p-1.5 rounded-lg hover:bg-[#ea580c] transition-colors"
                title="Add new bowl"
              >
                <Plus size={18} />
              </button>
        ...
        {!isAuthenticated ? (
          <div className="bg-white rounded-2xl p-6 shadow-lg border-2 border-gray-200">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="font-bold text-gray-800 text-lg">Admin Login</h3>
            </div>
            <p className="text-sm text-gray-500 mb-4">Only admin can add bowl points to the map</p>
            <form onSubmit={handleAdminLogin} className="flex gap-3">
              ...
              <button
                type="submit"
                className="px-6 py-3 bg-[#F97316] text-white rounded-xl font-bold hover:bg-[#ea580c] transition-colors shadow-md"
              >
                Login
              </button>
            </form>
          ...
          <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-2xl p-6 shadow-lg flex items-center justify-between text-white">
            <div>
              ...
              <p className="text-sm text-white/80">
                Click anywhere on the map, then press <strong>+</strong> to add a bowl point
              </p>
            </div>
            <button
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl">
            ...
            {clickedLatLng ? (
              <div className="mb-4 bg-green-50 border-2 border-green-200 rounded-xl px-4 py-2 flex items-center gap-2 text-sm text-green-700 font-bold">
                <MapPin size={16} />
                Location picked: {clickedLatLng.lat.toFixed(5)}, {clickedLatLng.lng.toFixed(5)}
              </div>
            ) : (
              <div className="mb-4 bg-yellow-50 border-2 border-yellow-200 rounded-xl px-4 py-2 text-sm text-yellow-700">
                💡 Close this modal and click on the map to pick exact location, or submit without clicking to place automatically.
              </div>
            )}
            ...
            <form onSubmit={handleAddBowl} className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={newBowlNote}
                  onChange={(e) => setNewBowlNote(e.target.value)}
                  placeholder="e.g., Under the tree, near bench"
                  className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-[#F97316] focus:outline-none bg-[#FFFBF5]"
                  required
                />
              </div>
              <div className="flex gap-3 pt-1">
                ...
                <button
                  type="submit"
                  className="flex-1 px-6 py-3 bg-[#F97316] text-white rounded-xl font-bold hover:bg-[#ea580c] transition-colors shadow-md"
                >
                  Add Bowl
                </button>
              </div>
